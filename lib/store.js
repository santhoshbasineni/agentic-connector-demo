...
import { regenerateAllSlots } from '@/lib/scheduling';
...
function initStore() {
  ...
  return {
    ...
    // Org practitioner directory (managed in the Org view's Config tab)
    practitioners: [
      { id: 'prac-1', name: 'Dr. Chen', specialty: 'Family Medicine', clinic: 'City Clinic' },
      { id: 'prac-2', name: 'Dr. Patel', specialty: 'Internal Medicine', clinic: 'City Clinic' },
      { id: 'prac-3', name: 'Dr. Ade', specialty: 'Orthopedics', clinic: 'Northside Practice' },
    ],
    // Recurring weekly availability, expanded into apptSlots on boot/save
    availabilityPatterns: [
      { id: 'pat-1', practitionerId: 'prac-1', days: [1, 3, 5], start: '09:00', end: '11:00', slotMinutes: 60, weeks: 3 },
      { id: 'pat-2', practitionerId: 'prac-2', days: [2, 4], start: '14:00', end: '16:00', slotMinutes: 60, weeks: 3 },
      { id: 'pat-3', practitionerId: 'prac-3', days: [5], start: '10:00', end: '12:00', slotMinutes: 60, weeks: 3 },
    ],
    // One-off blocked dates: [{ practitionerId, date: 'YYYY-MM-DD', reason }]
    blockedDates: [],
    // AU — expanded bookable slot instances (generated from the patterns above)
    apptSlots: [],
    payerRequests: [],
    appointments: [],
    eeAlerts: [],
...
export function getStore() {
  if (!globalThis.__agenticConnectorStore) {
    const s = initStore();
    globalThis.__agenticConnectorStore = s;
    regenerateAllSlots(s); // expand the seeded recurring patterns
  }
...
export function activeAppointments() {
  return getStore().appointments.filter((a) => a.status !== 'cancelled');
}
...
export function appointmentStatusSummary() {
  ...
  if (active.length === 0) {
    ...
    return `You have no appointments booked yet. ${free} clinic slot(s) are currently open — say "book an appointment" to see them.`;
  }
  const lines = active.map((a) =>
    a.facility === 'lab'
      ? `• ${a.id}: ${a.when} — lab specimen collection — confirmed (APC)`
      : `• ${a.id}: ${a.when} at ${a.clinic}${a.specialty ? ` (${a.specialty})` : ''} — confirmed (APC)`
  );
